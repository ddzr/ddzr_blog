...
```{r}
read_in_html_tbl %>%janitor::clean_names() %>%  filter(location %in% c('WILMINGTON, DE','WILMINGTON, DELAWARE') & case_status=='CERTIFIED') %>% 
...
read_in_html_tbl %>%janitor::clean_names() %>%  filter(location %in% c('WILMINGTON, DE','WILMINGTON, DELAWARE') & case_status=='CERTIFIED') %>% 
  mutate(salary=as.numeric(gsub(',','',base_salary)),
         start_date=lubridate::mdy(start_date),
         start_date_year=lubridate::year(start_date)
         ) %>% group_by(start_date_year) %>% summarise(n=n(),median_salary=scales::dollar(median(salary),accuracy = 1)) %>% arrange(start_date_year) %>% 
  head(10) %>%  kable() %>%
  kable_styling(bootstrap_options = "striped", full_width = F,position = "center")
```
...
#Takeaway 
This blog post aims to understand how H1B salary in chase, Wilmington distributed. Couple worth noting things： 
